<x-action-section>
    ...
    <x-slot name="content">
                        ...
                        <div class="ml-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('Last active') }} {{ Carbon\Carbon::createFromTimestamp($session->last_activity)->locale(str_replace('_', '-',
                                        app()->getLocale()))->diffForHumans() }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <form action="{{ route('admin.profile.devices.delete') }}" method="post">
                @csrf
                @method('DELETE')
                <x-button class="ml-2 text-xs">{{ __('Log Out Other Browser Sessions') }}</x-button>
            </form>
...
{{-- 
        ...
        <x-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="footer">
                <form action="{{ route('profile.devices.delete') }}" method="post">
                    @csrf
                    @method('DELETE')
                    <x-button class="ml-2">{{ __('Log Out Other Browser Sessions') }}</x-button>
                </form>
            </x-slot>
        </x-dialog-modal> --}}
    </x-slot>
