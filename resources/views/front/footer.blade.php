<footer class="w-full bg-gray-900">
    <div class="px-4 pt-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8">
        <div class="grid mb-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">
            <div class="sm:col-span-2">
                <a href="/" aria-label="{{ __('Go Home') }}" title="{{ config('app.name') }}" class="inline-flex items-center h-8">
                    <x-application-logo class="w-full h-full object-contain" />
                    <span class="ml-2 text-xl font-bold tracking-wide text-gray-200 uppercase">{{ config('app.name') }}</span>
                </a>
            ...
            <div class="space-y-2 text-sm">
                <p class="h-8 text-base font-bold tracking-wide text-gray-200">Contacts</p>
                <div class="mt-6">
                    <div class="flex">
                        <p class="mr-1 text-gray-200">{{ __('Phone') }} :</p>
                        <a href="tel:850-123-5021" aria-label="{{ __('Phone') }}" title="{{ __('Phone') }}" class="text-gray-200 transition-colors duration-500 hover:text-indigo-200">850-123-5021</a>
                    </div>
                    <div class="flex">
                        <p class="mr-1 text-gray-200">{{ __('Email') }} :</p>
                        <a href="mailto:info@lorem.mail" aria-label="{{ __('Email') }}" title="{{ __('Email') }}" class="text-gray-200 transition-colors duration-500 hover:text-indigo-200">info@lorem.mail</a>
                    </div>
                    <div class="flex">
                        <p class="mr-1 text-gray-200">{{ __('Address') }} :</p>
                        <a href="//maps.google.com" target="_blank" rel="noopener noreferrer nofollow" aria-label="{{ __('Address') }}" title="{{ __('Address') }}" class="text-gray-200 transition-colors duration-500 hover:text-indigo-200">
            ...
            <div>
                <span class="h-8 text-base font-bold tracking-wide text-gray-200">Social</span>
        ...
        <div class="flex flex-col-reverse items-center justify-between pt-5 pb-10 border-t border-gray-800 lg:flex-row">
            <p class="text-sm text-gray-200">
                &copy; Copyright {{ date('Y') }} {{ config('app.name') }}. {{ __('All rights reserved.') }}
            </p>
            <ul class="flex flex-wrap mb-3 gap-y-2 lg:mb-0 sm:gap-y-0 gap-x-5">
                <li>
                    <a href="#" class="text-sm text-gray-200 transition-colors duration-500 hover:text-indigo-200">F.A.Q</a>
                </li>
                <li>
                    <a href="{{ LaravelLocalization::getURLFromRouteNameTranslated(LaravelLocalization::getCurrentLocale(), 'routes.privacy') }}" class="text-sm text-gray-200 transition-colors duration-500 hover:text-indigo-200">{{ __('Privacy Policy') }}</a>
                </li>
                <li>
                    <a href="{{ LaravelLocalization::getURLFromRouteNameTranslated(LaravelLocalization::getCurrentLocale(), 'routes.terms') }}" class="text-sm text-gray-200 transition-colors duration-500 hover:text-indigo-200">{{ __('Terms of Service') }}</a>
                </li>
            </ul>
        </div>
    </div>
