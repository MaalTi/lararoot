<x-minimal-layout>
    <x-authentication-card class="space-y-4">
        ...
        <h1 class="text-xl font-semibold text-center">{{ __('Register') }}</h1>
        ...
        <form method="POST" action="{{ route('register') }}">
                ...
                <x-label for="terms">
                    <div class="flex items-center">
                        ...
                        <div class="ml-2">
                            ...
                            'terms_of_service' => '<a target="_blank" href="'.LaravelLocalization::getURLFromRouteNameTranslated(LaravelLocalization::getCurrentLocale(), 'routes.terms').'" class="text-sm text-gray-600 underline hover:text-gray-900">'.__('Terms of Service').'</a>',
                            'privacy_policy' => '<a target="_blank" href="'.LaravelLocalization::getURLFromRouteNameTranslated(LaravelLocalization::getCurrentLocale(), 'routes.privacy').'" class="text-sm text-gray-600 underline hover:text-gray-900">'.__('Privacy Policy').'</a>',
                            ]) !!}
                        </div>
                    </div>
                </x-label>
            ...
            <div class="flex items-center justify-between mt-4">
                <a class="text-sm text-gray-600 underline hover:text-blue-600" href="{{ route('login') }}">
        ...
        <div class="flex flex-wrap items-center justify-center w-full gap-2">
            <a href="{{ route('social.redirect','github') }}" class="flex items-center justify-center px-4 py-2 space-x-2 text-white transition-all duration-200 bg-black rounded-md hover:bg-opacity-80 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-1">
                ...
                <span> {{ __('Register with') }} Github </span>
            </a>
